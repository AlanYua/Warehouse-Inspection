/**
 * 日報表（已入庫）：依部門彙整，輸出 Excel。
 * - Sheet1: 單據彙總（類型 / 單據號碼 / 名稱 / 檢驗總數 / 箱數）
...
export async function buildDailyStockedWorkbook(args: {
  dateLabel: string;
  docs: DailyDocRow[];
  items: DailyItemRow[];
}): Promise<Uint8Array> {
  ...
  const sheetDocs = workbook.addWorksheet("已入庫-單據", {
  ...
  sheetDocs.columns = [
    ...
    { header: "箱數", key: "packageCount", width: 10 },
  ];

  sheetDocs.addRow([`日報表（已入庫）`, `日期：${args.dateLabel}`]);
  sheetDocs.mergeCells(1, 1, 1, 6);
  ...
  const sheetItems = workbook.addWorksheet("已入庫-品項", {
    views: [{ state: "frozen", ySplit: 2 }],
  });
  sheetItems.columns = [
    { header: "部門", key: "departmentName", width: 14 },
  ...
  sheetItems.addRow([`日報表（已入庫）`, `日期：${args.dateLabel}`]);
